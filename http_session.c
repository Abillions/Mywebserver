...
//获得用户的请求uri:/index.html?name=zhiwei
...
int get_http_process(int client_sfd, char *recvbuf){
	...
	get_uri(recvbuf, uribuf);//  例如：/index.html?name=sunzhiwei
	transcode(uribuf);//转码
	get_path(uribuf, pathbuf);// 例如：/index.html
	printf("客户端请求的路径:%s\n", pathbuf);
	if(strcmp(pathbuf, "/")==0)//默认访问的是index.html页面
		strcpy(pathbuf, "/web-app/mysite/index.html");
...
void serve_static(int client_sfd, char *uribuf, char *pathbuf){
	...
	if(fd<0)
		goto open_err;
	int fsize = lseek(fd, 0, SEEK_END);	//文件长度
	lseek(fd, 0, SEEK_SET);
